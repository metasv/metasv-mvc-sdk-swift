//
// ContractFtAddressTx.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation

/** Transaction history of the address for specific ft. */
public struct ContractFtAddressTx: Codable {

    /** Paging flag */
    public var flag: String?
    /** The address of the record */
    public var address: String?
    /** Specific codeHash */
    public var codeHash: String?
    /** specific genesis */
    public var genesis: String?
    /** timestamp of the tx */
    public var time: Int64?
    /** Block Height of the tx, -1 if not confirmed */
    public var height: Int64?
    /** total income of the address from this tx */
    public var income: Int64?
    /** total outcome of the address from this tx */
    public var outcome: Int64?
    /** txid of this record */
    public var txid: String?

    public init(flag: String? = nil, address: String? = nil, codeHash: String? = nil, genesis: String? = nil, time: Int64? = nil, height: Int64? = nil, income: Int64? = nil, outcome: Int64? = nil, txid: String? = nil) {
        self.flag = flag
        self.address = address
        self.codeHash = codeHash
        self.genesis = genesis
        self.time = time
        self.height = height
        self.income = income
        self.outcome = outcome
        self.txid = txid
    }

}

